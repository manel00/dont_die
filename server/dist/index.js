...
const CORE_TOOLS = [
    ...
    {
        name: "godot_command",
        description: "Send a custom command to Godot bridge. This is a low-level escape hatch when a named tool is not enough.",
        inputSchema: {
            type: "object",
            properties: {
...
function toCompatInputPayload(rawArgs) {
        ...
        return ALL_COMPAT_TOOLS.filter((tool) => {
            ...
            for (const cat of LITE_CATEGORIES) {
                if (TOOL_INPUT_SCHEMAS[tool.name])
                    return true; // Keep tools with schemas
            }
            return false;
